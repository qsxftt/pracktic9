        ...
        for item in data.get("items", []):
            try:
                ...
                vacancy_detail = requests.get(
                    f"https://api.hh.ru/vacancies/{item['id']}", timeout=10
                )
                ...
                if vacancy_detail.status_code == 200:
                    ...
                    if vacancy_data.get("specializations"):
                        specialization = vacancy_data["specializations"][0].get(
                            "name", ""
                        )
                    ...
                    vacancy, created = Vacancy.objects.update_or_create(
                        ...
                        defaults={
                            ...
                            "company_name": vacancy_data.get("employer", {}).get(
                                "name", ""
                            ),
                            "company_logo": vacancy_data.get("employer", {})
                            .get("logo_urls", {})
                            .get("90"),
    ...
    if request.method == "POST":
        ...
        if search_form.is_valid():
            ...
            search_query_obj = SearchQuery.objects.create(
                query=search_text, area=area, per_page=per_page
            )
            ...
            messages.success(
                request, f'Найдено {len(vacancies)} вакансий по запросу "{search_text}"'
            )
            ...
            if vacancies:
                ...
                if salary_max:
                    filtered_vacancies = [
                        v
                        for v in filtered_vacancies
                        if v.salary_to and v.salary_to <= salary_max
                    ]
